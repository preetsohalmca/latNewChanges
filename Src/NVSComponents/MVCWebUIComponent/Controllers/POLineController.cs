    ...
    public class POLineController : BaseController
    {
            ...
            bool isRenewalYes = false, bool isRenewalNo = false, bool isRenewalAll = false)
        {
            ...
            else
            {
                ...
                {
                    var pageRecords = request.Page * request.PageSize;
        ...
        public JsonResult SaveDetail(POLineSaveViewModel pOLineSave)
        {
            ...
            if (result != null)
            {
                ...
                return this.Json(new { statusO = true, renewaDaysLeft = renwalDaysLeft }, JsonRequestBehavior.AllowGet);
            }

            return this.Json(new { statusO = false, renewaDaysLeft = renwalDaysLeft }, JsonRequestBehavior.AllowGet);
        }
                ...
                if (records.ToList().Count() > 0)
                {
                    foreach (var item in records)
                    {
                        ...
                        InvoiceReportRecordDTO obj = new InvoiceReportRecordDTO
                        {
                            ...
                            CostCenter = item.CostCenter.CountryCode + "" + item.CostCenter.FullName,
        ...
        public JsonResult FillInvoiceReport()
        {
            var fillInvoiceReportObjList = this.InvoiceReportServices.GetInvoicingReports()
                                    .OrderByDescending(x => x.TimeStamp)
                                    .Select(x => new { InvoiceReportID = x.InvoicingReportID, Year = x.Year, TimeStamp = x.TimeStamp.ToString("MM/dd/yyyy HH:mm:ss.fff", CultureInfo.InvariantCulture), Month = x.Month, InvoiceReportId = x.InvoicingReportID })
                                    .ToList();
                        ...
                        for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                        {
                            ...
                            if (workSheet.Cells[rowIterator, 2].Value != null)
                            {
                                ...
                                if (purchaseOrderLine == null && purchaseOrderFromDb.PurchaseOrderId == Guid.Empty)
                                {
                                    ...
                                    purchaseOrderLine.IsNewPurchaseOrder = true;
                                    if (ownerlist != null)
                                    {
                                ...
                                if (purchaseOrderLine == null && purchaseOrderFromDb.PurchaseOrderId != Guid.Empty)
                                {
                                    ...
                                    if (purchaseOrderLine.PurchaseOrderLineFromEbd.PurchaseOrderLineFromEbdId == Guid.Empty)
                                    {
                                        purchaseOrderLine.PurchaseOrderLineFromEbd.PurchaseOrderLineFromEbdId = Guid.NewGuid();
                                    }
                                    purchaseOrderLine.PurchaseOrderLineFromEbd.PurchaseOrder = new PurchaseOrder();
                                    purchaseOrderLine.PurchaseOrderLineFromEbd.PurchaseOrder = purchaseOrderFromDb;
                                ...
                                purchaseOrderLine.PurchaseOrderLineFromEbd.LineItemDescription = Convert.ToString(workSheet.Cells[rowIterator, 5].Value);
                                purchaseOrderLine.PurchaseOrderLineFromEbd.PurchaseOrder.PurchaseOrderName = Convert.ToString(workSheet.Cells[rowIterator, 6].Value);
                                ...
                                var internalOrderId = Convert.ToString(workSheet.Cells[rowIterator, 14].Value.ToString());
                                ...
                                purchaseOrderLine.PurchaseOrderLineFromEbd.LowestBorg = Convert.ToString(workSheet.Cells[rowIterator, 18].Value);
                                purchaseOrderLine.RequestorName = Convert.ToString(workSheet.Cells[rowIterator, 19].Value.ToString());
                                purchaseOrderLine.PurchaseOrderLineFromEbd.RequesterEmail = Convert.ToString(workSheet.Cells[rowIterator, 20].Value);
                                purchaseOrderLine.PurchaseOrderLineFromEbd.PurchaserName = Convert.ToString(workSheet.Cells[rowIterator, 21].Value);
                                purchaseOrderLine.PurchaseOrderLineFromEbd.FunctionalApproverName = Convert.ToString(workSheet.Cells[rowIterator, 22].Value);
                                purchaseOrderLine.PurchaseOrderLineFromEbd.PurchaseOrder.VendorName = Convert.ToString(workSheet.Cells[rowIterator, 23].Value);
                                ...
                                purchaseOrderLine.PurchaseOrderLineFromEbd.LicenseType = Convert.ToString(workSheet.Cells[rowIterator, 29].Value);
                                purchaseOrderLine.PurchaseOrderLineFromEbd.SoftwareName = Convert.ToString(workSheet.Cells[rowIterator, 30].Value);
                                ...
                                if (purchaseOrderLine.IsNewPurchaseOrder)
                                {
                                    this.PurchaseOrderService.Save(purchaseOrderLine.PurchaseOrderLineFromEbd.PurchaseOrder);
                                }
                                poLines.Add(purchaseOrderLine);

                            }

                            result = this.POLineServices.InserBulk(poLines);
                        }
