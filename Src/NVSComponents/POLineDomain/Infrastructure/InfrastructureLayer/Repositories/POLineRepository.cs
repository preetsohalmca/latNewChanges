...
namespace Volvo.LAT.POLineDomain.InfrastructureLayer.Repositories
{
    ...
    public class POLineRepository : GenericRepository<POLine>, IPOLineRepository
    {
        ...
        public IEnumerable<string> FindAllOwner(string ownerName = null)
        {
            ...
            var test = this.Session.QueryOver<POLine>().List(); ;
            //.Select(testRequest => new { testRequest.OwnerName })
            // .OrderBy(o => o.OwnerName);

            //.Where(testRequest => testRequest.OwnerName )
        ...
        public bool SaveUpdateDetail(POLine pLine, string purchaseOrderComment)
        {
            using (var transaction = this.Session.BeginTransaction())
            {
                try
                {
                    ...
                    // if (!string.IsNullOrEmpty(purchaseOrderComment))
                    //  {
                    //pLine.PurchaseOrderLineFromEbd.PurchaseOrder.InvoiceNumber = pLine.PurchaseOrderLineFromEbd.PurchaseOrder.InvoiceNumber;
                    pLine.PurchaseOrderLineFromEbd.PurchaseOrder.Comments = purchaseOrderComment;
                    // }
                    //pLine.PurchaseOrderLineFromEbd.CostCenter = pLine.CostCenter;
                    //pLine.PurchaseOrderLineFromEbd.CostCenterOLD = pLine.CostCenter.FullName;
        ...
        public List<CustomModelSecondGrid> RetrieveDetailInnerGridData(string poLineId)
        {
            ...
            if (poLineData.Count() > 0)
            {
                ...
                //var wbsData = this.Session.QueryOver<WbsElement>().List();
                var poLineList = poLineData = poLineData;
        ...
        public bool BulkInsert(List<POLine> polines)
        {
            using (var transaction = this.Session.BeginTransaction())
            {
                try
                {
                    var purchaseOrder = polines.Where(x => x.IsNewPurchaseOrder).Select(x => x.PurchaseOrderLineFromEbd.PurchaseOrder).GroupBy(x=>x.PoNumber).ToList();

                    this.Session.SetBatchSize(1000);
